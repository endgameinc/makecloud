...
let missing_ami_err_msg =
  "AWS failed to generate an ami when requested, this in theory shouldn't \
   happen."
...
module Aws : Provider_template.Provider = struct
  ...
  let apply_tags ~(settings : Settings.t) ~aws_key ~aws_secret ?token ~guid
      ~instance_id () =
    ...
    let tags_req =
      Types.CreateTagsRequest.make ~resources:[ instance_id ] ~tags ()
    in
    let%lwt result =
      Aws_lwt.Runtime.run_request ~region:settings.aws_region
        ~access_key:aws_key ~secret_key:aws_secret ?token
        (module CreateTags)
        tags_req
    in
    Lwt.return (result |> aws_to_result)

  let get_ami ~(params : Lib.run_parameters) ~(settings : Settings.t) ~guid
      past_stage =
    let%lwt creds =
      Aws_s3_lwt.Credentials.Helper.get_credentials ?profile:params.aws_profile
        ()
    in
    ...
    in
    Lwt.return
    @@ R.reword_error (fun _ -> `Msg "Failed to get ami in s3.") result
  ...
  let spinup (params : Lib.run_parameters) (settings : Settings.t)
      (n : Node.real_node) guid : t Lwt.t =
    let%lwt () = Node.node_log n "Node Spinning Up" in
    (* TODO improve this to new cred method? maybe pass creds around *)
    let%lwt unsafe_creds =
      Aws_s3_lwt.Credentials.Helper.get_credentials ?profile:params.aws_profile
        ()
    in
    ...
    let instance_params =
      ...
      Types.RunInstancesRequest.make ~image_id:base ~min_count:1 ~max_count:1
        ~key_name:settings.aws_key_name
        ~security_group_ids:[ settings.aws_security_group ]
        ~instance_type
        ~block_device_mappings:
          [
            Types.BlockDeviceMapping.make ~device_name:"/dev/xvda"
              ~ebs:
                (Types.EbsBlockDevice.make ~volume_size:disk_size
    ...
    let get_instance_id () =
      let%lwt result =
        Aws_lwt.Runtime.run_request ~region:settings.aws_region
          ~access_key:aws_key ~secret_key:aws_secret ?token:aws_token
          (module RunInstances)
          instance_params
      in
      Lwt.return (result |> aws_to_result)
    ...
    let%lwt _t =
      repeat_until_ok
        (apply_tags ~settings ~aws_key ~aws_secret ~instance_id ~guid)
        5
    in
    let get_details () =
      let result =
      ...
      | ( `Ok
          ...
          let%lwt () = Node.node_log n (sprintf "ip addr: %s" ip) in
          Lwt.return_ok ip
      | `Ok _, _ -> Lwt.return (R.error_msg "Can't get an ip for this box yet.")
      | (`Error _ as e), _ -> Lwt.return (aws_to_result e)
    in
    (*TODO aws_retry*)
    let%lwt ip = repeat_until_ok get_details 40 in
    let ip_address = R.failwith_error_msg ip in
    Lwt.return
      { ip_address; instance_id; settings; aws_key; aws_secret; aws_token }
  ...
  let wait_until_ready t (n : Node.real_node) () =
    ...
    | Result.Ok _ ->
        ...
        Lwt.return (Some ())
    | exception _e -> Lwt.return None
    | _ -> Lwt.return None

  (*TODO Please no more string types.*)
  let send_command box s ~expire_time :
    ...
    let send_command () =
      ...
      let process_response x =
        ...
        | `Accepted | `OK -> Lwt.return (R.ok (resp, body))
        | `Unauthorized ->
            Lwt.return
              (R.error
                 ( "Unauthorized",
    ...
    (*TODO cohttp_retry*)
    let poll_agent () =
      let check_uri = Uri.of_string ("http://" ^ box ^ ":8000/check_command") in
      ...
      | resp, body -> (
          ...
          | `Accepted -> Lwt.return (R.error ("still waiting.", ""))
          | `OK -> Lwt.return (R.ok output)
          | `Unprocessable_entity ->
              Lwt.return (R.error ("command failed on agent.", output))
          | _ -> Lwt.return (R.error ("some other error.", "some other error."))
          )
    in
    repeat_until_ok poll_agent expire_time

  let save_box ~t ~(settings : Settings.t) ~(n : Node.real_node) ~guid
      ~instance_id () =
    ...
    let%lwt result =
      Aws_lwt.Runtime.run_request ~region:settings.aws_region
        ~access_key:t.aws_key ~secret_key:t.aws_secret ?token:t.aws_token
        (module CreateImage)
        image_req
    in
    Lwt.return (result |> aws_to_result)

  let store_ami ?profile ~(settings : Settings.t) ~(n : Node.real_node) ~guid
      ami_id =
    ...
    in
    Lwt.return
    @@ R.reword_error (fun _ -> `Msg "Failed to store ami in s3.") result
  ...
  let check_on_ami ~t ~(settings : Settings.t) ~n image_id () =
    ...
    let%bind result =
      Lwt.Infix.(
        Aws_lwt.Runtime.run_request ~region:settings.aws_region
          ...
          ami_req
        >>= fun x -> Lwt.return (aws_to_result x))
    in
    ...
    let _ =
      Node.node_log n
        (Fmt.str "Checking on ami %s with state %s." image_id
           (Types.ImageState.to_string ami.state))
    in
    Lwt.return
    @@
    match ami.state with
    | Available -> Ok image_id
    | (Pending | Transient) as e ->
        R.error_msgf "AMI is still not ready with reason %s."
          (Types.ImageState.to_string e)
    | (Invalid | Deregistered | Failed | Error) as e ->
        R.error_msgf "AMI has failed with reason %s."
          (Types.ImageState.to_string e)
  ...
  let publish_image ?profile ~t ~settings ~n ~guid () =
    ...
    let bind = Lwt_result.bind in
    let%bind box_id =
      repeat_until_ok (save_box ~t ~settings ~n ~instance_id ~guid) 20
    in
    let none () = R.error_msg missing_ami_err_msg in
    let%bind image_id =
      Types.CreateImageResult.(box_id.image_id)
      |> R.of_option ~none |> Lwt.return
    in
    let%bind waiting_image =
      repeat_until_ok (check_on_ami ~t ~settings ~n image_id) 240
    in
    let%bind _store_result =
      store_ami ?profile ~settings ~n ~guid waiting_image
    in
    Lwt.return_ok waiting_image

  let runcmd transfer t (params : Lib.run_parameters) (settings : Settings.t)
      (n : Node.real_node) guid (cmd : Command.t) :
      (string, [> R.msg ] * string) result Lwt.t =
    ...
    | Publish -> (
        let%lwt image_id =
          publish_image ?profile:params.aws_profile ~t ~settings ~n ~guid ()
        in
        match image_id with
        | Ok i ->
            let%lwt () = Node.node_log n (Fmt.str "Saved instance as %s" i) in
            Lwt.return_ok i
        | Error e -> Lwt.return_error (e, missing_ami_err_msg))

  let spindown t (n : Node.real_node) =
    let%lwt () = Node.node_log n "Node spinning down." in
    let details () =
      Aws_lwt.Runtime.run_request ~region:t.settings.aws_region
        ~access_key:t.aws_key ~secret_key:t.aws_secret ?token:t.aws_token
