...
let get_assoc_list x =
  match x with
  | `O y -> R.ok y
  | _ -> R.error (R.msg "node must have list of attributes below it.")
...
let get_value assoc_list key =
  ...
  match results with
  | hd :: _ -> R.ok hd
  | _ -> R.error (R.msg ("key " ^ key ^ " isn't found in the assoc list"))

let get_string str =
  match str with
  | `String x -> R.ok (String.trim x)
  | _ -> R.error (R.msg "not a string")
