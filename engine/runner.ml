...
let files_per_dir cwd =
  let rec aux dir_handle configs : string list Lwt.t =
    ...
    | ".." -> aux dir_handle configs
    | x -> aux dir_handle (Filename.concat cwd x :: configs)
...
let transfer_to_shell ~(transfer_fn : string -> [ `Get | `Put ] -> Uri.t)
  ...
  let transfer ~first_arg ~second_arg ~(verb : [ `Get | `Put ]) =
    ...
    | `Get, true ->
        sprintf
          {|powershell -command "$ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest '%s' -Method 'GET' -UseBasicParsing -OutFile %s"|}
          get_url second_arg
    | `Put, false ->
        sprintf "curl --retry 5 -X PUT \"%s\" --upload-file %s" put_url
          first_arg
    | `Put, true ->
        sprintf
          {|powershell -command "$ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest '%s' -Method 'PUT' -UseBasicParsing -Infile %s"|}
          put_url first_arg
  in
  transfer
...
module Runner (M : Provider_template.Provider) = struct
  let run_node ~(settings : Settings.t) ~params ~n
    ...
    let prep_steps =
      if Node.rnode_has_keyword n Windows then
        [
          sprintf
            {|powershell -command "$ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest '%s' -OutFile C:\source.tar"|}
            uri_str;
    ...
    in
    let%lwt () =
      if not (List.exists (String.equal n.name) params.dont_delete) then
        M.spindown box n
      else
        let%lwt () =
          Node.node_log n
            (Fmt.str "NOT deleting box %s as per requested." n.name)
        in
        Lwt.return ()
    in
    match result with
  ...
  (*TODO Retry this.*)
  let store_logs ~(params : Lib.run_parameters) ~(settings : Settings.t) ~guid
      ~(n : Node.real_node) ~console_logs =
    let%lwt creds =
  ...
  let store_cache ~(params : Lib.run_parameters) ~(settings : Settings.t) ~guid
      ~cwd ~n =
  ...
  let transfer_file ~(params : Lib.run_parameters) ~(settings : Settings.t)
      ~old_guid ~new_guid ~(n : Node.real_node) filename :
  ...
  let process_cache ~params ~(settings : Settings.t) ~old_guid ~new_guid
      ~(n : Node.real_node) =
    let%lwt () = Lwt_io.printl (n.color ^ "Processing cache for " ^ n.name) in
    let key = Sys.getenv "MC_KEY" in
    let%lwt () =
      Notify.send_state ~settings ~guid:new_guid ~node:(Node.Rnode n) ~key
        ProcessCache
    in
  ...
  let invoke settings (params : Lib.run_parameters) (n : Node.real_node)
    ...
    let%lwt cache_status =
      check_cache ?profile:params.aws_profile ~settings ~cwd:params.repo_dir ~n
        ()
    in
    ...
    | true -> (
        ...
        match%lwt
          process_cache ~params ~settings ~old_guid:cache_guid ~new_guid:guid ~n
        with
    ...
    | false -> (
        ...
        | Ok console_logs ->
            ...
            let%lwt () =
              store_cache ~params ~settings ~guid ~cwd:params.repo_dir ~n
            in
...
let ok_or_raise_aws = function
  ...
  | `Error e ->
      raise
        (Generic_AWS_Error
           (Aws.Error.format Aws_ec2.Errors_internal.to_string e))
...
let invoke_node settings params node
  ...
  let run node =
    ...
    | Node.Rnode r -> AwsRunner.invoke settings params r transfer_fn
  in
...
let run settings params node_list
  ...
  let rec aux complete_nodes failed_nodes
    ...
    | [], [], false -> Lwt.return (complete_nodes, failed_nodes, todo_nodes)
    | [], td, false -> Lwt.return (complete_nodes, failed_nodes, td)
    | running_nodes, todo_nodes, _ ->
        let%lwt done_nodes, still_running =
          match running_nodes with
          | [] -> Lwt.return ([], [])
          | _ -> Lwt.nchoose_split running_nodes
        in
        ...
        let running =
          List.map
            (fun n ->
              let%lwt r = invoke_node settings params n transfer_fn in
              Lwt.return (r, n))
            runnable
        in
        aux
...
let pre_presign ~params ~bucket ~region ~duration =
  ...
  let date =
    ...
    | Some x -> x
    | None ->
        raise (Incorrect_Time_Format "Current time isn't formatted correctly!?")
  in
  let region = Aws_s3.Region.of_string region in
  Lwt.return (fun path (verb : [ `Get | `Put ]) ->
      Aws_s3.Authorization.make_presigned_url ~credentials ~date ~region ~bucket
        ~duration ~path ~verb ())
...
let pre_source (settings : Settings.t) cwd guid
  ...
  let%lwt _pout =
    ...
    Lwt_process.pread (Lwt_process.shell cmd)
  in
  let upload_url =
    transfer_fn (sprintf "/%s/source.tar" (Uuidm.to_string guid)) `Put
  in
  let upload_cmd =
    sprintf "curl --retry 5 -X PUT \'%s\' --upload-file %s"
      (Uri.to_string upload_url) temp
  in
  let%lwt _pout = Lwt_process.pread (Lwt_process.shell upload_cmd) in
...
let prune_nodes ns target_nodes =
  ...
  let rec aux edges (targets : string list) =
    let valid_edges =
      List.filter (fun (dst, _) -> List.exists (String.equal dst) targets) edges
    in
    let new_targets =
      List.fold_left
        (fun ts (_, src) ->
...
let main (params : run_parameters) =
  ...
  in
  let%lwt transfer_fn =
    pre_presign ~params ~bucket:settings.storage_bucket ~duration:86400
  ...
  let%lwt pre_results =
    pre_source settings params.repo_dir params.guid transfer_fn
  in
  let key = Sys.getenv "MC_KEY" in
  ...
  let%lwt () =
    Notify.send_run_state ~settings ~guid ~key
      (Notify.make_run_start node_names edges)
  in
  match%lwt run settings params runable_nodes transfer_fn with
  | exception e ->
      let%lwt () =
        Notify.send_run_state ~settings ~guid ~key Notify.RunException
      in
      let%lwt () =
        Lwt_io.printf
  ...
  | _, [], [] ->
      let%lwt () =
        Notify.send_run_state ~settings ~guid ~key Notify.RunSuccess
      in
  ...
  | complete, failed, todo ->
      ...
      in
      let todo_names = List.map Node.node_to_string todo |> String.concat " " in
      let%lwt () =
        Lwt_io.printl
          (sprintf
             "\n\n\
