...
let keyword_of_string k =
  ...
  | [ "expire" ] ->
      R.error_msg
        (sprintf "%s needs a argument, i.e. expire-30 or expire-60." k)
  | [ "disk"; ds ] -> (
  ...
  | [ "boxtype" ] ->
      R.error_msg (sprintf "%s needs a argument, i.e. boxtype <box type>" k)
  | _ -> R.error_msg (sprintf "%s is not a known keyword." k)
...
(*
let rnode_get_keyword (n : real_node) keyword = 
  List.filter ()
  *)
...
let hash_of_folder folder =
  let cmd =
    sprintf "find %s -type f -print0 | sort -z | xargs -0 sha256sum | sha256sum"
      folder
  in
...
let hash_of_node cwd (n : real_node) =
  let%lwt file_hash =
    Lwt_list.map_p (fun x -> hash_of_folder (Filename.concat cwd x)) n.file_root
  in
...
let dependson_count n =
  match n with
  | Rnode { dependson = d; _ } | Snode { dependson = d; _ } -> List.length d
...
let get_edges n =
  match n with
  | Rnode r -> List.map (fun x -> (x, r.name)) r.dependson
  | Snode s -> List.map (fun x -> (x, s.name)) s.dependson
